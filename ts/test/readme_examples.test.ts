...
//     (\`new RandomUserGeneratorSDK(...)\` / \`RandomUserGeneratorSDK.test(...)\` become
//     \`RandomUserGeneratorSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'RandomUserGeneratorSDK'
...
const TEST_SEED = {"entity":{"get_random_user":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new RandomUserGeneratorSDK(...)\` and any
// \`RandomUserGeneratorSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+RandomUserGeneratorSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/RandomUserGeneratorSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
