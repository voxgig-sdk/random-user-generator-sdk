...
// RandomUserGenerator SDK struct utility test
//
...
class StructUtilityTest extends TestCase
{
    ...
    {
        if (self::$run === null) {
            $runner = RandomUserGeneratorOmni::makeStructRunner(
    ...
    {
        return RandomUserGeneratorOmni::objmodel(RandomUserGeneratorOmni::argval($val));
    }

    /** A single-case fixture value on the comparison side. */
    private static function outval($val)
    {
        return RandomUserGeneratorOmni::unmark($val);
    }

    /** A result normalised to the comparison model. */
    private static function resval($val)
    {
        return RandomUserGeneratorOmni::structfix($val, Struct::undef());
    }
        ...
        $this->runsection(
            ...
            function ($input) {
                ...
                if (is_object($store) && $haspath && Struct::undef() !== $val) {
                    ...
                    if (json_encode(RandomUserGeneratorOmni::structfix($now, Struct::undef()))
                        !== json_encode(RandomUserGeneratorOmni::structfix($val, Struct::undef()))) {
