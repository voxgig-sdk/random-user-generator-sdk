...
// RandomUserGenerator SDK retry feature
...
class RandomUserGeneratorRetryFeature extends RandomUserGeneratorBaseFeature
{
    ...
    public function init(RandomUserGeneratorContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (RandomUserGeneratorContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(RandomUserGeneratorContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(RandomUserGeneratorContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
