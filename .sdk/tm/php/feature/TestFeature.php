...
// RandomUserGenerator SDK test feature

require_once __DIR__ . '/BaseFeature.php';
require_once __DIR__ . '/../utility/Param.php';

class RandomUserGeneratorTestFeature extends RandomUserGeneratorBaseFeature
{
    ...
    public function init(RandomUserGeneratorContext $ctx, array $options): void
    {
        ...
        $test_fetcher = function (RandomUserGeneratorContext $fctx, string $_fullurl, array $_fetchdef) use ($entity): array {
            ...
            if ($op->name === 'load') {
                ...
                $id = RandomUserGeneratorParam::call($fctx, 'id');
    ...
    public function makeNetsim(array $net, callable $inner): callable
    {
        ...
        return function (RandomUserGeneratorContext $fctx, string $url, array $fetchdef) use ($net, $inner, $pick_latency, $sleep): array {
            $this->_netcalls++;
            $call = $this->_netcalls;
     ...
     */
    public function buildArgs(RandomUserGeneratorContext $ctx, $op, $args): array
    {
        ...
        foreach ($keys as $k) {
            ...
            if ($is_id || $in_reqd) {
                $v = RandomUserGeneratorParam::call($ctx, $k);
