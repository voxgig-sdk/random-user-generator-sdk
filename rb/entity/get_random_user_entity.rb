...
class GetRandomUserEntity
  ...
  def make
    opts = @_entopts.dup
    GetRandomUserEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = RandomUserGeneratorHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [GetRandomUser, Hash] the current GetRandomUser data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = RandomUserGeneratorHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of GetRandomUser fields)
  def match_get
  ...
  #
  # @param reqmatch [GetRandomUserListMatch, Hash, nil] match filter (any subset of
  #   GetRandomUser fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<GetRandomUser>, Array] the matching GetRandomUser items; raises RandomUserGeneratorError on failure
