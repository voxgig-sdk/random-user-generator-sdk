...
require_relative 'RandomUserGenerator_types'
...
class RandomUserGeneratorSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = RandomUserGeneratorHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = RandomUserGeneratorHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, RandomUserGeneratorFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    RandomUserGeneratorUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = RandomUserGeneratorHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = RandomUserGeneratorHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = RandomUserGeneratorHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def op_denied(op)
    ...
    {
      "ok" => false,
      "err" => RandomUserGeneratorError.new(
        "#{op}_allow",
  ...
  def raw_request(fetchargs = {})
    ...
    rescue RandomUserGeneratorError => err
    ...
    if fetched.is_a?(Hash)
      status = RandomUserGeneratorHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = RandomUserGeneratorError.new(
        "graphql_error", "RandomUserGeneratorSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = RandomUserGeneratorSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
