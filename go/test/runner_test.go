...
func envOverride(m map[string]any) map[string]any {
	if os.Getenv("RANDOMUSERGENERATOR_TEST_LIVE") == "TRUE" ||
		os.Getenv("RANDOMUSERGENERATOR_TEST_OVERRIDE") == "TRUE" {
		for key := range m {
	...
	if explain := os.Getenv("RANDOMUSERGENERATOR_TEST_EXPLAIN"); explain != "" {
