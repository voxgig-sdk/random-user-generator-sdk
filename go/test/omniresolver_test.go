...
import (
	...
	omni "github.com/voxgig-sdk/random-user-generator-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.RandomUserGeneratorError); is {
				out := map[string]any{
					"name":    "RandomUserGeneratorError",
...
func omniCtx(arg any, client *sdk.RandomUserGeneratorSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.RandomUserGeneratorSDK); is {
			client = live
