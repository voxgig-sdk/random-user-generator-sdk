...
func TestGetRandomUserEntity(t *testing.T) {
	...
	t.Run("stream", func(t *testing.T) {
		...
		var seen []any
		for item := range base.GetRandomUser(nil).Stream("list", nil, nil) {
		...
		if hasStreaming {
			...
			var got []any
			for item := range streamSdk.GetRandomUser(nil).Stream("list", nil, nil) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		getRandomUserRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.get_random_user")))
...
func get_random_userBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("RANDOM_USER_GENERATOR_TEST_GET_RANDOM_USER_ENTID")
	...
	idmapResolved := core.ToMapAny(env["RANDOM_USER_GENERATOR_TEST_GET_RANDOM_USER_ENTID"])
