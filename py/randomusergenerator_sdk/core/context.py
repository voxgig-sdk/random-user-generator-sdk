...
from randomusergenerator_sdk.core.helpers import get_ctx_prop, to_map
...
class RandomUserGeneratorContext:
    def __init__(self, ctxmap=None, basectx=None):
        ...
        # Ctrl
        self.ctrl = RandomUserGeneratorControl()
        ...
        if isinstance(sp, RandomUserGeneratorSpec):
            self.spec = sp
        elif basectx is not None:
        ...
        if isinstance(r, RandomUserGeneratorResult):
            self.result = r
        elif basectx is not None:
        ...
        if isinstance(rp, RandomUserGeneratorResponse):
            self.response = rp
        elif basectx is not None:
    ...
    def resolve_op(self, opname):
        ...
        if opname == "":
            return RandomUserGeneratorOperation({})
        ...
        op = RandomUserGeneratorOperation({
            "entity": entname,
            "name": opname,
            "input": inpt,
            "points": points,
    ...
    def make_error(self, code, msg):
        return RandomUserGeneratorError(code, msg, self)
