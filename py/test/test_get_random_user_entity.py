...
class TestGetRandomUserEntity:
    ...
    def test_should_stream(self):
        ...
        base = RandomUserGeneratorSDK.test(seed, None)
        seen = list(base.GetRandomUser(None).stream("list", None, None))
        assert len(seen) == 3
        ...
        if isinstance(cfg.get("feature"), dict) and "streaming" in cfg["feature"]:
            sdk = RandomUserGeneratorSDK.test(
                seed, {"feature": {"streaming": {"active": True}}})
            got = []
            for item in sdk.GetRandomUser(None).stream("list", None, None):
    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set RANDOM_USER_GENERATOR_TEST_GET_RANDOM_USER_ENTID JSON to run live")
        client = setup["client"]
...
def _get_random_user_basic_setup(extra):
    ...
    idmap_resolved = helpers.to_map(
        env.get("RANDOM_USER_GENERATOR_TEST_GET_RANDOM_USER_ENTID"))
    if idmap_resolved is None:
        idmap_resolved = helpers.to_map(idmap)

    if env.get("RANDOM_USER_GENERATOR_TEST_LIVE") == "TRUE":
    ...
    _live = env.get("RANDOM_USER_GENERATOR_TEST_LIVE") == "TRUE"
    return {
        "client": client,
        "data": entity_data,
        "idmap": idmap_resolved,
        "env": env,
        "explain": env.get("RANDOM_USER_GENERATOR_TEST_EXPLAIN") == "TRUE",
        "live": _live,
