# random-user-generator-cli

boru-driven command-line client **and** interactive REPL for the RandomUserGenerator
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/random-user-generator-cli)
make build

# 2. See usage (words, entities, env vars)
./random-user-generator-cli --help

# 3. Provide credentials once, via the environment
export RANDOM_USER_GENERATOR_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./random-user-generator-cli list get_random_user

# 5. Override the API base URL for a single call
RANDOM_USER_GENERATOR_BASE=https://api.example.com ./random-user-generator-cli list get_random_user

# 6. No arguments -> interactive REPL
./random-user-generator-cli
random-user-generator> list get_random_user
random-user-generator> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/random-user-generator-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/random-user-generator-cli list get_random_user
   ```
...
Run with no arguments to open a REPL (prompt `random-user-generator>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `get_random_user`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/random-user-generator-cli`. |
...
(`./random-user-generator-cli <expr>`) and interactively (the REPL), and expressions compose the
