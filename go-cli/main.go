// Command RandomUserGenerator-cli is a CLI + REPL for the RandomUserGenerator SDK,
...
const prompt = "random-user-generator"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "get_random_user"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      RANDOM_USER_GENERATOR_APIKEY (api key), RANDOM_USER_GENERATOR_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: RANDOM_USER_GENERATOR_APIKEY carries the API key and
	// RANDOM_USER_GENERATOR_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("RANDOM_USER_GENERATOR_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("RANDOM_USER_GENERATOR_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewRandomUserGeneratorSDK(opts)
