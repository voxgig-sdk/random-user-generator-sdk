...
// RandomUserGenerator SDK feature test
//
...
class FtCtrl extends RandomUserGeneratorControl
{
...
class FtClient
{
    ...
    private RandomUserGeneratorUtility $utility;

    public function __construct(RandomUserGeneratorUtility $utility, array $options)
    {
    ...
    public function get_utility(): RandomUserGeneratorUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (RandomUserGeneratorContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public RandomUserGeneratorContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (RandomUserGeneratorContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = RandomUserGeneratorConfig::make_feature($name);
    ...
    private static function buildUrl(RandomUserGeneratorSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof RandomUserGeneratorError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new RandomUserGeneratorResponse($fetched);
        }
    ...
    private function fail(RandomUserGeneratorContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (RandomUserGeneratorContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
