...
// RandomUserGenerator SDK pipeline test
//
...
{
    private static function utility(): RandomUserGeneratorUtility
    {
        return new RandomUserGeneratorUtility();
    }
    ...
    private static function ctx(array $over = []): RandomUserGeneratorContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new RandomUserGeneratorContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new RandomUserGeneratorOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof RandomUserGeneratorError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new RandomUserGeneratorOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = RandomUserGeneratorMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new RandomUserGeneratorOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = RandomUserGeneratorMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new RandomUserGeneratorOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = RandomUserGeneratorMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new RandomUserGeneratorSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = RandomUserGeneratorMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new RandomUserGeneratorResponse([]),
            'result' => new RandomUserGeneratorResult([]),
        ]);
        [, $err] = RandomUserGeneratorMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new RandomUserGeneratorSpec([]),
            'response' => null,
            'result' => new RandomUserGeneratorResult([]),
        ]);
        [, $err] = RandomUserGeneratorMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new RandomUserGeneratorSpec(['step' => 's']),
            'response' => new RandomUserGeneratorResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new RandomUserGeneratorResult([]),
        ]);
        [, $err] = RandomUserGeneratorMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new RandomUserGeneratorSpec(['step' => 's']),
            'response' => new RandomUserGeneratorResponse(self::resp(200, ['v' => 1])),
            'result' => new RandomUserGeneratorResult([]),
        ]);
        [, $err] = RandomUserGeneratorMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new RandomUserGeneratorSpec(['step' => 's']),
            'response' => new RandomUserGeneratorResponse(self::resp(200, ['v' => 2])),
            'result' => new RandomUserGeneratorResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        RandomUserGeneratorMakeResponse::call($ctx);
    ...
    {
        $preset = new RandomUserGeneratorResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new RandomUserGeneratorSpec([]),
            'response' => new RandomUserGeneratorResponse([]),
            'result' => new RandomUserGeneratorResult([]),
        ...
        [$got, $err] = RandomUserGeneratorMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new RandomUserGeneratorResult([])]);
        [, $err] = RandomUserGeneratorMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new RandomUserGeneratorSpec([]), 'result' => null]);
        [, $err] = RandomUserGeneratorMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new RandomUserGeneratorSpec(['step' => 's']),
            'result' => new RandomUserGeneratorResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new RandomUserGeneratorOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = RandomUserGeneratorMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new RandomUserGeneratorSpec(['step' => 's']),
            'result' => new RandomUserGeneratorResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new RandomUserGeneratorOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = RandomUserGeneratorMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new RandomUserGeneratorResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new RandomUserGeneratorSpec([]),
            'result' => new RandomUserGeneratorResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = RandomUserGeneratorMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = RandomUserGeneratorMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new RandomUserGeneratorError('boom', 'boom');
        $utility->fetcher = function (RandomUserGeneratorContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new RandomUserGeneratorSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = RandomUserGeneratorMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (RandomUserGeneratorContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = RandomUserGeneratorMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(RandomUserGeneratorResponse::class, $response);
    ...
    public function test_make_request_records_the_fetchdef_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->explain = ['on' => true];
        RandomUserGeneratorMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (RandomUserGeneratorContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new RandomUserGeneratorSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = RandomUserGeneratorMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new RandomUserGeneratorResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new RandomUserGeneratorSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = RandomUserGeneratorMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = RandomUserGeneratorMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = RandomUserGeneratorMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new RandomUserGeneratorResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, RandomUserGeneratorDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new RandomUserGeneratorResult(['ok' => false])]);
        $this->expectException(RandomUserGeneratorError::class);
        RandomUserGeneratorDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new RandomUserGeneratorResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', RandomUserGeneratorMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        RandomUserGeneratorMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new RandomUserGeneratorBaseFeature();
        $b = new RandomUserGeneratorBaseFeature();
        RandomUserGeneratorFeatureAdd::call($ctx, $a);
        RandomUserGeneratorFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): RandomUserGeneratorBaseFeature
    {
        $f = new RandomUserGeneratorBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        RandomUserGeneratorFeatureAdd::call($ctx, self::named_feature('a'));
        RandomUserGeneratorFeatureAdd::call($ctx, self::named_feature('b'));
        $this->assertSame(['a', 'b'], $names());

        $before = self::named_feature('z1');
        $before->_options = ['__before__' => 'b'];
        RandomUserGeneratorFeatureAdd::call($ctx, $before);
        $this->assertSame(['a', 'z1', 'b'], $names());

        $after = self::named_feature('z2');
        $after->_options = ['__after__' => 'a'];
        RandomUserGeneratorFeatureAdd::call($ctx, $after);
        $this->assertSame(['a', 'z2', 'z1', 'b'], $names());

        $replace = self::named_feature('z3');
        $replace->_options = ['__replace__' => 'z1'];
        RandomUserGeneratorFeatureAdd::call($ctx, $replace);
        ...
        RandomUserGeneratorFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): RandomUserGeneratorContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new RandomUserGeneratorSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = RandomUserGeneratorPrepareAuth::call($ctx);
    ...
    public function test_prepare_auth_a_raw_apikey_goes_in_as_is(): void
    {
        ...
        RandomUserGeneratorPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new RandomUserGeneratorResponse(['headers' => null]),
            'result' => new RandomUserGeneratorResult([]),
        ]);
        RandomUserGeneratorResultHeaders::call($ctx);
    ...
    public function test_result_body_skips_parsing_when_the_body_is_absent(): void
    {
        ...
        RandomUserGeneratorResultBody::call($ctx);
