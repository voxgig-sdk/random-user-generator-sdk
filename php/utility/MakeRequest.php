...
class RandomUserGeneratorMakeRequest
{
    public static function call(RandomUserGeneratorContext $ctx): array
    {
        ...
        if ($fetch_err) {
            ...
            $response = new RandomUserGeneratorResponse(['err' => $ctx->make_error('request_no_response', 'response: undefined')]);
        } elseif (is_array($fetched)) {
            $response = new RandomUserGeneratorResponse($fetched);
        } else {
