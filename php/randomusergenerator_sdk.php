...
class RandomUserGeneratorSDK
{
    ...
    public function __construct(array $options = [])
    {
        ...
        $feature_opts = RandomUserGeneratorHelpers::to_map(Struct::getprop($this->options, "feature"));
        if ($feature_opts) {
            $featureorder = Struct::getpath($this->options, "__derived__.featureorder");
            if (is_array($featureorder)) {
                foreach ($featureorder as $fname) {
                    $fopts = RandomUserGeneratorHelpers::to_map($feature_opts[$fname] ?? null);
                    if ($fopts && isset($fopts["active"]) && $fopts["active"] === true) {
                        ...
                        if (!RandomUserGeneratorFeatures::has_feature($fname)) {
                        ...
                        ($utility->feature_add)($this->_rootctx, RandomUserGeneratorFeatures::make_feature($fname));
                    }
                }
            }
        }
    ...
    {
        return RandomUserGeneratorUtility::copy($this->_utility);
    }
    ...
    public function prepare(array $fetchargs = []): mixed
    {
        ...
        $ctrl = RandomUserGeneratorHelpers::to_map(Struct::getprop($fetchargs, "ctrl")) ?? [];
        ...
        $params = RandomUserGeneratorHelpers::to_map(Struct::getprop($fetchargs, "params")) ?? [];
        $query = RandomUserGeneratorHelpers::to_map(Struct::getprop($fetchargs, "query")) ?? [];
    ...
    private function op_denied(string $op): array
    {
        ...
        return [
            "ok" => false,
            "err" => new RandomUserGeneratorError($op . "_allow",
                "RandomUserGeneratorSDK: " . $op . ": operation not allowed by" .
    ...
    private function raw_request(array $fetchargs = []): mixed
    {
        ...
        if (is_array($fetched)) {
            $status = RandomUserGeneratorHelpers::to_int(Struct::getprop($fetched, "status"));
    ...
    public function graphql(string $query, ?array $variables = null, ?array $ctrl = null): mixed
    {
        ...
        if (is_array($errors) && 0 < count($errors)) {
            ...
            $res["ok"] = false;
            $res["err"] = new RandomUserGeneratorError("graphql_error",
                "RandomUserGeneratorSDK: graphql: " . $msg);
            $res["graphql"] = $errors;
        }
    ...
    public function GetRandomUser($data = null)
    {
        ...
        if ($data === null) {
            if ($this->_get_random_user === null) {
                $this->_get_random_user = new GetRandomUserEntity($this, null);
            }
            return $this->_get_random_user;
        }
        return new GetRandomUserEntity($this, $data);
    }
    ...
    public static function test(?array $testopts = null, ?array $sdkopts = null): self
    {
        ...
        $sdk = new RandomUserGeneratorSDK($sdkopts);
        $sdk->mode = "test";
        return $sdk;
    }
}
