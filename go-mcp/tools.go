...
type Args struct {
	Entity string         `json:"entity" jsonschema:"get_random_user"`
	Query  map[string]any `json:"query,omitempty" jsonschema:"optional match map e.g. {\"id\":1} for load, omit for list"`
}

func registerTools(server *mcp.Server, client *sdk.RandomUserGeneratorSDK) {
	mcp.AddTool(server, &mcp.Tool{
		Name: "random-user-generator_list",
		Description: "List records from RandomUserGenerator. " +
	...
	mcp.AddTool(server, &mcp.Tool{
		Name: "random-user-generator_load",
		Description: "Load a single record from RandomUserGenerator. " +
...
func runOp(client *sdk.RandomUserGeneratorSDK, op string, args Args) (*mcp.CallToolResult, any, error) {
...
func entityFor(client *sdk.RandomUserGeneratorSDK, name string) (sdk.RandomUserGeneratorEntity, error) {
	switch strings.ToLower(name) {
	case "get_random_user":
		return client.GetRandomUser(nil), nil
