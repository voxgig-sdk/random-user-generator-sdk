...
[MCP](https://modelcontextprotocol.io) server exposing the RandomUserGenerator SDK as
two agent tools — `random-user-generator_list` and `random-user-generator_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/random-user-generator-mcp)
make build

# 2. Provide credentials via the environment
export RANDOM_USER_GENERATOR_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user random-user-generator \
  -- /absolute/path/to/random-user-generator-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./random-user-generator-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// random-user-generator_list: first page of records
{ "entity": "get_random_user" }
{ "entity": "get_random_user", "query": { } }

// random-user-generator_load: one record by id
{ "entity": "get_random_user", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/random-user-generator-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user random-user-generator \
     -- "$PWD"/dist/*/random-user-generator-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `random-user-generator_list` and `random-user-generator_load` tools now appear
   in new sessions. Ask the agent to *"list get_random_user using random-user-generator"*
   and it calls `random-user-generator_list` with `{"entity":"get_random_user"}`.
...
```sh
export RANDOM_USER_GENERATOR_APIKEY=sk_live_xxx            # API key
export RANDOM_USER_GENERATOR_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `random-user-generator_list` tool
...
### Call the `random-user-generator_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `random-user-generator_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `random-user-generator_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `RANDOM_USER_GENERATOR_APIKEY` | API key sent with every request. |
| `RANDOM_USER_GENERATOR_BASE` | Optional override of the API base URL. |
...
```sh
./random-user-generator-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"random-user-generator_load","arguments":{"entity":"get_random_user","query":{"id":1}}}}'
```
